...
| Shared grammar Phase 4, first family: `OccurrenceUsagePrefix` | Complete for `OccurrenceUsage`, `ItemUsage`, `SatisfyRequirementUsage`, `PartUsage`; every other family listed with what it still needs | [occurrence-usage-prefix-matrix.md](../planning/occurrence-usage-prefix-matrix.md) |
| Shared grammar Phase 4, `PartUsage` slice | Complete — all thirteen owning scopes, five construction paths and six competing productions; `PortUsage` is the recommended next candidate | [part-usage-prefix-matrix.md](../planning/part-usage-prefix-matrix.md) |
