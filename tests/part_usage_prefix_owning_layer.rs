//! Owning-layer mechanics of `PartUsage`'s `OccurrenceUsagePrefix`.
//!
//! What the prefix *parses to* is pinned by the semantic snapshots
//! (`tests/snapshots/sysml/part_usage_prefix_alternatives.md`, `..._owning_scopes.md`,
...
//!
//! The grammar is `planning/part-usage-prefix-matrix.md`.
...
fn every_prefix_slot_round_trips_and_formats_idempotently() {
    for source in [
        ...
        "package P {\n    snapshot part p;\n}\n",
        "package P {\n    timeslice part p;\n}\n",
        "package P {\n    individual snapshot part p;\n}\n",
        "package P {\n    individual timeslice part p;\n}\n",
        // The whole prefix, in the only legal order.
        "package P {\n    in derived abstract constant ref individual snapshot part p : T;\n}\n",
        "package P {\n    out variation ref individual timeslice part p : T;\n}\n",
        // Extension keywords: one, several, qualified, absolute, quoted.
        "package P {\n    #Tag part p;\n}\n",
        "package P {\n    #First #Second part p;\n}\n",
        "package P {\n    #Lib::Tag part p;\n}\n",
        "package P {\n    #$::P::Tag part p;\n}\n",
        "package P {\n    #'safety critical' part p;\n}\n",
        "package P {\n    abstract ref #Tag part p : T;\n}\n",
        // `MemberPrefix` visibility is the membership's, and precedes the prefix.
        "package P {\n    private ref individual part p : T;\n}\n",
        "package P {\n    protected #Tag part p : T;\n}\n",
        // Every declaration shape the `Usage` tail reaches, with a prefix in front of it.
        "package P {\n    ref part <sn> p : T;\n}\n",
        "package P {\n    ref part : T;\n}\n",
        "package P {\n    ref part :>> target;\n}\n",
        "package P {\n    ref part :>> target : T;\n}\n",
        "package P {\n    derived part p : T[0..*] ordered nonunique;\n}\n",
...
fn strict_and_editor_agree_on_clean_prefixed_input() {
    let source = concat!(
        ...
        "    part def T;\n",
        "    in derived abstract constant ref individual snapshot part p : T;\n",
        "    out variation ref individual timeslice part q : T;\n",
        "    #Tag #P::Tag part r : T;\n",
        "    private ref part s : T;\n",
        "    part def Owner {\n",
        "        snapshot part inDefinition;\n",
        "        #Tag part tagged : T;\n",
        "    }\n",
        "}\n",
    );
...
/// A prefix whose `part` never arrives must not leave its speculative references behind.
///
/// A `UsageExtensionKeyword` allocates an arena entry for its qualified name before the parser can
/// know whether `part` follows. If a refused parse kept them, the arena would grow entries the
...
#[test]
fn a_refused_part_prefix_leaves_no_arena_entry() {
    ...
    for refused_source in [
        ...
        "package P {\n    part def A {\n        #Ghost 123;\n    }\n}\n",
        // A complete prefix with no `part` after it.
        ...
        "package P {\n    part def A {\n        snapshot timeslice part p;\n    }\n}\n",
        // `part def` after a usage-only prefix is a definition this parser must refuse, not a
        // usage named `def`.
        "package P {\n    part def A {\n        ref part def B;\n    }\n}\n",
    ] {
        ...
        assert_eq!(
            ...
            baseline,
            "a refused part prefix left speculative references in the arena:\n{refused_source}"
        );
    }
}
...
#[cfg(feature = "serde")]
#[test]
fn a_part_prefix_keyword_span_covering_other_text_is_rejected() {
...
#[cfg(feature = "serde")]
#[test]
fn a_part_direction_span_covering_a_different_keyword_is_rejected() {
    let document = parse_for_editor("package P {\n    inout part p;\n}\n").document;
    let mut tampered = serde_json::to_value(&document).expect("the parsed document serializes");
    let direction = "/root/elements/0/value/Package/value/body/Brace/elements/0\
                     /value/PartUsage/value/prefix/basic/ref_prefix/direction";
    *tampered
...
#[cfg(feature = "serde")]
#[test]
fn a_part_variance_span_covering_a_different_keyword_is_rejected() {
    let document = parse_for_editor("package P {\n    abstract part p;\n}\n").document;
    let mut tampered = serde_json::to_value(&document).expect("the parsed document serializes");
    let variance = "/root/elements/0/value/Package/value/body/Brace/elements/0\
                    /value/PartUsage/value/prefix/basic/ref_prefix/variance";
    *tampered
...
#[cfg(feature = "serde")]
#[test]
fn a_part_portion_span_covering_a_different_keyword_is_rejected() {
    let document = parse_for_editor("package P {\n    snapshot part p;\n}\n").document;
    let mut tampered = serde_json::to_value(&document).expect("the parsed document serializes");
    let portion = "/root/elements/0/value/Package/value/body/Brace/elements/0\
                   /value/PartUsage/value/prefix/portion";
    *tampered
...
#[cfg(feature = "serde")]
#[test]
fn part_prefix_slots_written_out_of_order_are_rejected() {
    let document = parse_for_editor("package P {\n    ref individual part p;\n}\n").document;
    let mut tampered = serde_json::to_value(&document).expect("the parsed document serializes");
    let prefix = "/root/elements/0/value/Package/value/body/Brace/elements/0\
                  /value/PartUsage/value/prefix";
...
#[cfg(feature = "serde")]
#[test]
fn a_part_extension_keyword_sigil_covering_other_text_is_rejected() {
    let document = parse_for_editor("package P {\n    #Tag part p;\n}\n").document;
    let mut tampered = serde_json::to_value(&document).expect("the parsed document serializes");
    let sigil = "/root/elements/0/value/Package/value/body/Brace/elements/0\
                 /value/PartUsage/value/prefix/extension_keywords/0/value/hash_span";
    *tampered
...
#[cfg(feature = "serde")]
#[test]
fn a_dangling_part_extension_keyword_reference_is_rejected() {
    let document = parse_for_editor("package P {\n    #Tag part p;\n}\n").document;
    let mut tampered = serde_json::to_value(&document).expect("the parsed document serializes");
    let annotation = "/root/elements/0/value/Package/value/body/Brace/elements/0\
                      /value/PartUsage/value/prefix/extension_keywords/0/value/annotation";
    *tampered
...
#[cfg(feature = "serde")]
#[test]
fn a_part_prefix_span_outside_its_own_declaration_is_rejected() {
    let document =
...
#[cfg(feature = "serde")]
#[test]
fn a_part_prefix_document_from_another_schema_version_is_rejected() {
    let document = parse_for_editor("package P {\n    ref individual part p;\n}\n").document;
    let mut tampered = serde_json::to_value(&document).expect("the parsed document serializes");
    *tampered
...
#[cfg(feature = "serde")]
#[test]
fn a_prefixed_part_document_round_trips_through_the_envelope() {
    let source = concat!(
        "package P {\n",
        "    metadata def Tag;\n",
        "    part def T;\n",
        "    in derived abstract constant ref individual snapshot #Tag #P::Tag part p : T {\n",
        "        attribute a;\n",
        "    }\n",
        "    out variation ref individual timeslice #Tag part q : T;\n",
        "    private part <sn> r : T[0..*] ordered nonunique :> p = q;\n",
        "    ref part :>> p : T;\n",
        "}\n",
    );
