...
| --- | --- | --- |
| **`OccurrenceUsagePrefix` on a satisfy usage** — the production's own first element: `RefPrefix` (`in`/`out`/`inout`, `derived`, `abstract`/`variation`, `constant`, `ref`), `individual`, a `PortionKind` (`snapshot`/`timeslice`), and `UsageExtensionKeyword*` | `SatisfyRequirementUsage` → `OccurrenceUsagePrefix` (SysML BNF line 564) | **Not implemented, and the AST has nowhere to retain these authored facts.** This is a third part of the authoritative production left unmet, distinct from the owning-scope gap below. It is absent from the whole pinned corpus, and closing it properly means a shared typed occurrence-usage prefix node rather than a satisfy-local one, since every `OccurrenceUsageElement` spells the same prefix. A `MemberPrefix` visibility keyword is likewise not modelled |
